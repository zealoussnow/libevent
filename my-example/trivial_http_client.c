// Last Update:2015-11-06 11:04:28
...
int main(int argc, char **argv)
{
	...
	bufferevent_enable(bev, EV_READ|EV_WRITE); // bufferevent默认启用写事件
	evbuffer_add_printf(bufferevent_get_output(bev), "GET %s\r\n", argv[2]);
	printf("bufferevent_get_enabled: %d\n", bufferevent_get_enabled(bev));
	if (bufferevent_get_enabled(bev) & EV_READ)
	{
		printf("bufferevent enable reading!!!\n");
	}
	if (bufferevent_get_enabled(bev) & EV_WRITE)
	{
		printf("bufferevent enable writing!!!\n");
	}
